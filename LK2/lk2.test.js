...
describe("LK2", () => {
  describe("DOM", () => {
    ...
    `;
      ...
      const button = document.getElementById('a'); //angepasst wir müssen die id zuerst holen

      expect(button.id).toBe("a");
      expect(button.tagName).toBe("BUTTON"); // angepasst
  ...
  describe("Events", () => {
    ...
    `;
      ...
      try {
        ...
        //click durch handeDocumentClick ersetzt
      }
    });
  });
  ...
  describe("Forms and Inputs", () => {
    test("Find named element in a form", () => {
      document.body.innerHTML = `
      <form id="vehicles">
        <input id="firstName" type="text" disabled> <!-- Id wurde zu id="firstName" korrigiert -->
  ...
  describe("Web components", () => {
    ...
    `;
      ...
      const myElement = document.getElementsByTagName("my-element")[0]; // destruktor entfernt
    
      myElement.attributeChangedCallback('title', '', 'This is my excellent title.');
    
      await customElements.whenDefined("my-element"); // auf element warten wegen aysnc
  ...
  return cookie;
    ...
    test("get key/value pairs", () => {
      ...
      // test angepasst damit es mit dem snapshot übereinstimmt
  ...
  describe("Storage", () => {
    ...
    test("get values in object", () => {
      ...
      // nein, darum haben wir es oben hinzugefügt
